...
#include "object_binning_parallel.h"

namespace embree
{
  template<int logBlockSize>
  __forceinline ObjectBinningParallel<logBlockSize>::ObjectBinningParallel(const BuildRange& range, size_t target, Box* prims)
    : BuildRange(range), target(target), bestDim(0), bestSplit(0)
  {
    ...
    scale = rcp(embree::size(centBounds)) * ssef((float)numBins);
  ...
  void ObjectBinningParallel<logBlockSize>::stage0(size_t elt)
  {
    ...
    ssize_t start = elt*size()/8, end = (elt+1)*size()/8;
    Thread& t = thread[elt];
