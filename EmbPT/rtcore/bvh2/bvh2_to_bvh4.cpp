...
namespace embree
{
  Ref<BVH4<Triangle4> > BVH2ToBVH4::convert(Ref<BVH2<Triangle4> >& bvh2)
  {
    ...
    stream.precision(0);
    stream << "conversion time = " << (t1-t0)*1000.0f << " ms" << std::endl;
    stream.setf(std::ios::scientific, std::ios::floatfield);
    stream.precision(2);
    stream << "sah = " << bvh4->getSAH() << std::endl;
    stream.setf(std::ios::fixed, std::ios::floatfield);
    stream.precision(1);
    stream << "size = " << (bytesNodes+bytesTris)/1048576.0f << " MB" << std::endl;
    stream << "nodes = "  << bvh4->getNumNodes() << " "
           << "(" << bytesNodes/1048576.0f << " MB) "
           << "(" << 100.0*(bvh4->getNumNodes()-1+bvh4->getNumLeaves())/(4.0*bvh4->getNumNodes()) << " %)" 
           << std::endl;
    stream << "leaves = " << bvh4->getNumLeaves() << " "
           << "(" << bytesTris/1048576.0f  << " MB) "
           << "(" << 100.0*bvh4->getNumPrims()/(4.0*bvh4->getNumPrimBlocks()) << " %)" 
           << std::endl;
    std::cout << stream.str();
    return bvh4;
  }
