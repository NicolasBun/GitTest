...
#include <sstream>
#include "bvh2_builder_spatial.h"
...
namespace embree
{
  Ref<BVH2<Triangle4> > BVH2BuilderSpatial::build(const BuildTriangle* triangles, size_t numTriangles)
  {
    ...
    stream << "leaves = " << bvh->getNumLeaves() << " "
           ...
           << std::endl;
    stream << "prims = " << bvh->getNumPrims() << " "
           << "(" << 100.0f*float(bvh->getNumPrims())/float(numTriangles) << " %)" 
           << std::endl;
    std::cout << stream.str();
    return bvh;
  }
  ...
  __forceinline BVH2BuilderSpatial::BuildTaskLow::BuildTaskLow(BVH2BuilderSpatial* parent,
  ...
  __forceinline BVH2BuilderSpatial::BuildTaskHigh::BuildTaskHigh(BVH2BuilderSpatial* parent,
                                                                 size_t primBegin, size_t primEnd,
                                                                 size_t jobBegin, size_t jobEnd, size_t numJobs)
