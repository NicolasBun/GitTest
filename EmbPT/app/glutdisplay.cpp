...
namespace embree
{
  ...
  void snapshot(const FileName& filename) {
    /* store to disk */
    ...
    std::cout << "storeImage" << std::endl;
    storeImage(image.cast<Image>(),filename);
        std::cout << "end storeImage" << std::endl;
  ...
  void keyboardFunc(unsigned char k, int, int)
  {
    switch (k)
    {
      ...
      case 'L' :
      {
		  	std::cout << "radius " <<  g_camRadius << std::endl;
			g_camRadius += 1;
		  
	}
	break;
      case '+' : g_speed *= 1.2f; std::cout << "speed = " << g_speed << std::endl; break;
      case '-' : g_speed /= 1.2f; std::cout << "speed = " << g_speed << std::endl; break;
  ...
  {
    Vec3f deltaVec,viewVec,strafeVec;
    float mult = 1.;
    ...
    switch (k) {
      case GLUT_KEY_RIGHT :
        mult = -1;
      case GLUT_KEY_LEFT :
        viewVec = normalize(g_camLookAt - g_camPos);
        strafeVec = cross(g_camUp, viewVec);
        deltaVec = strafeVec * mult * g_speed;
        g_camPos += deltaVec;
  ...
  void displayFunc(void)
  {
    ...
    g_renderer->rtSetBool1("linedraw",g_lineDrawing);
    g_renderer->rtCommit();
    ...
    /* measure rendering time, fps and set window title */
    ...
    glutSetWindowTitle(stream.str().c_str());
  }
