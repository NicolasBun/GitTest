...
namespace embree
{
  ...
  {
    ssize_t ofs = atol(xml->parm("ofs").c_str());
    ssize_t size = atol(xml->parm("size").c_str());
  ...
  template<> std::vector<Vec2f> XMLImporter::load<std::vector<Vec2f> >(const Ref<XML>& xml)
  {
    ...
    ssize_t size = xml->body.size();
    if (size % 2 != 0) throw std::runtime_error(xml->loc.str()+": wrong vector<float2> body");
    std::vector<Vec2f> vec(size/2);
    for (ssize_t i=0; i<size/2; i++) vec[i] = Vec2f(xml->body[2*i+0].Float(),xml->body[2*i+1].Float());
    return vec;
  }
  ...
  template<> std::vector<float3> XMLImporter::load<std::vector<float3> >(const Ref<XML>& xml)
  {
    ...
    for (ssize_t i=0; i<size/3; i++) vec[i] = float3(xml->body[3*i+0].Float(),xml->body[3*i+1].Float(),xml->body[3*i+2].Float());
    return vec;
  }
  ...
  template<> std::vector<Vec3i> XMLImporter::load<std::vector<Vec3i> >(const Ref<XML>& xml)
  {
    ...
    for (ssize_t i=0; i<size/3; i++) vec[i] = Vec3i(xml->body[3*i+0].Int(),xml->body[3*i+1].Int(),xml->body[3*i+2].Int());
    return vec;
  }
  ...
  Ref<Device::RTMaterial> XMLImporter::loadMaterial(const Ref<XML>& xml)
  {
    ...
    for (ssize_t i=0; i < (ssize_t)parms->children.size(); i++) {
  ...
  Ref<Scene> XMLImporter::loadTriangleMesh(const Ref<XML>& xml)
  {
    ...
    if (positions.size()) mesh->rtSetArray("positions","float3",&positions[0],positions.size(),sizeof(float3));
    if (triangles.size()) mesh->rtSetArray("indices"  ,"int3"  ,&triangles[0],triangles.size(),sizeof(Vec3i));
  ...
  Ref<Scene> XMLImporter::loadTransformNode(const Ref<XML>& xml)
  {
    ...
    for (ssize_t i=1; i < (ssize_t)xml->children.size(); i++)
  ...
  Ref<Scene> XMLImporter::loadGroupNode(const Ref<XML>& xml)
  {
    ...
    for (ssize_t i=0; i < (ssize_t)xml->children.size(); i++)
      node->children.push_back(loadScene(xml->children[i]));
    return node;
  }
