...
namespace embree
{
  ...
  *******************************************************************/
  ...
  /** create a new scene. */
  Ref<Device::RTScene> Device::rtNewScene(const char* type, Ref<RTPrimitive>* prims_i, size_t size) {
    embree::RTPrimitive* prims = new embree::RTPrimitive[size];
    for (size_t i=0; i<size; i++) prims[i] = (embree::RTPrimitive) prims_i[i]->handle;
    Ref<Device::RTScene> scene = new Device::RTScene (this,embree::rtNewScene(type,prims,size));
  ...
  *******************************************************************/
  ...
  void Device::RTHandle::rtSetTextureStack(const char* property, std::vector<Ref<RTTexture> > texstack) {
		std::vector<embree::RTTexture > tmp_stack;
		for(std::vector<Ref<RTTexture> >::iterator it =texstack.begin();it != texstack.end(); it++) {
			tmp_stack.push_back((embree::RTTexture)((*it)->handle));
		}
    embree::rtSetTextureStack((embree::RTHandle)handle,property,tmp_stack);
  }
