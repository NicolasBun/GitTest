...
namespace embree
{
  ...
  Ref<Scene> load(const FileName& fileName)
  {
    ...
    //if (ext == "obj" ) return loadObj (fileName);
		if (ext == "obj" ) return loadAI (fileName);
    if (ext == "xml" ) return loadXML (fileName);
    if (ext == "lwo" ) return loadAI (fileName);
    if (ext == "3ds" ) return loadAI (fileName);
    if (ext == "dxf" ) return loadAI (fileName);
    if (ext == "rif")  return loadRif(fileName);
    if (ext ==  "dae")  return loadAI (fileName);
