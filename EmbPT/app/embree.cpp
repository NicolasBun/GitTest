...
namespace embree
{
  ...
  Ref<Device::RTCamera> createCamera(const AffineSpace& space)
  {
    ...
    camera->rtSetTransform("world2local",space);
  ...
  static Ref<Device::RTRenderer> parseMaterialRenderer(Ref<ParseStream> cin, const FileName& path)
  {
    Ref<Device::RTRenderer> renderer = g_device->rtNewRenderer("material");
    if (g_depth >= 0) renderer->rtSetInt1("maxDepth",g_depth);
    ...
    while (cin->peek() != "}") {
      ...
      cin->force("=");
      if (tag == "depth") renderer->rtSetInt1("maxDepth",cin->getInt());
      else std::cout << "unknown tag \"" << tag << "\" in material renderer parsing" << std::endl;
    }
    cin->drop();
  ...
  static Ref<Device::RTRenderer> parseOpenGLRenderer(Ref<ParseStream> cin)
  {
    Ref<Device::RTRenderer> renderer = g_device->rtNewRenderer("opengl");
    if (g_depth >= 0) renderer->rtSetInt1("maxDepth",g_depth);
		renderer->rtSetInt1("width",g_width);
		renderer->rtSetInt1("height",g_height);
  ...
  static Ref<Device::RTRenderer> parsePathTracer(Ref<ParseStream> cin, const FileName& path)
  {
    ...
    while (cin->peek() != "}") {
      ...
      else if (tag == "integrator"     ) renderer->rtSetString("integrator"     ,cin->getString().c_str());
