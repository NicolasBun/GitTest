...
namespace embree
{
  IntegratorRenderer::IntegratorRenderer(const Parms& parms)
  {
    ...
    if (_integrator == "pathtracer") integrator = new PathTraceIntegrator(parms);
		else if (_integrator == "directlighting") integrator = new DirectLightingIntegrator(parms);
...
#ifdef SEE_ACHIEVMENT
			if (tile%(numTiles/10) == 0) {
				int level = (tile*10/(float)numTiles)+0.5;
				std::cerr<<"\rAchievment : "<<level*10<<" %";//<<std::endl;
				if (level == 10) std::cerr<<std::endl;
			}
#endif
		}
