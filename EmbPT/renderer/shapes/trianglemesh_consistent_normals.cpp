...
namespace embree
{
  ...
  struct InputData3 {
    ...
    const char* position;				//!< Array of coordinates
    size_t stridePositions;			//!< Stride for positions
    const char* normal  ;				//!< Array of normals
    size_t strideNormals;				//!< Stride for normals
    const char* texcoord;				//!< Array of tex coordinates
    size_t strideTexCoords;			//!< Stride for tex coord
  };
  ...
  void TriangleMeshConsistentNormals::postIntersect(const Ray& ray, DifferentialGeometry& dg) const
  {
    ...
    dg.Ng = normalize(cross(p1 - p0,p2 - p0));
