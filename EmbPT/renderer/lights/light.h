...
namespace embree
{
  ...
  public:
    ...
    virtual Ref<Light> transform (const AffineSpace& xfm) const = 0;

    /*! Evaluates the radiance that would arrive at a given location
     *  from a given direction assuming no blocking geometry.
     ...
     * \returns the radiance arriving from the sampled direction. */
  ...
  public:
    ...
    /*! Returns the emitted radiance for a location on the light
     *  and direction. */
