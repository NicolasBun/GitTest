...
#include "textures/assimptexture.h"
...
namespace embree
{
  ...
  *******************************************************************/
  ...
  RT_API_SYMBOL RTTexture rtNewTexture(const char* type) {
    ...
    else if (!strcasecmp(type,"assimp")) return (RTTexture) new NormalHandle<AssimpTexture,Texture>;
  ...
  RT_API_SYMBOL RTMaterial rtNewMaterial(const char* type)
  {
    ...
    else if (!strcasecmp(type,"Assimp")        ) return (RTMaterial) new NormalHandle<Assimp,Material>;
  ...
  RT_API_SYMBOL RTRenderer rtNewRenderer(const char* type)
  {
    ...
    else if (!strcasecmp(type,"opengl")) {
			// ADVICE : to send handlers (sample / integrator) : do it NOW and HERE
			return (RTRenderer) new NormalHandle<OpenGLRenderer,Renderer>;
		}
    else if (!strcasecmp(type,"material")) {
			// ADVICE : to send handlers (sample / integrator) : do it NOW and HERE
			return (RTRenderer) new NormalHandle<MaterialRenderer,Renderer>;
		}
  ...
  *******************************************************************/
  ...
  RT_API_SYMBOL void rtSetTextureStack(RTHandle handle, const char* property, std::vector<RTTexture> tx_stack) {
    Lock<MutexSys> lock(*mutex);
    verifyInitialized();
    if (!handle  ) throw std::runtime_error("invalid handle"  );
    if (!property) throw std::runtime_error("invalid property");
		// We must convert RTTexture to Texture
		std::vector<Ref<Texture> > tmp_stack;
		BaseHandle<Texture>* texture;
		for(std::vector<RTTexture >::iterator it =tx_stack.begin();it != tx_stack.end(); it++) {
			texture = castHandle<BaseHandle<Texture> >(*it,"texture");
			tmp_stack.push_back(texture->instance);
		}

    handle->set(property,Variant(tmp_stack));
  }
