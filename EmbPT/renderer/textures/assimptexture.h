...
#ifndef __EMBREE_ASSIMP_TEXTURE_H__
#define __EMBREE_ASSIMP_TEXTURE_H__
...
namespace embree
{
   ...
   *  lookup comming from Assimp import library. */
  class AssimpTexture : public Texture
  {
  public:

    /*! Construction from parameters. */
    AssimpTexture (const Parms& parms) {
      image = parms.getImage("image");
			strength = parms.getFloat("strength",1.);
    }
  ...
  private:
    ...
    float strength;		//!< Strength of the image
  };
}
