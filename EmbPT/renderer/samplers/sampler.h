...
namespace embree
{
   ...
   *  The sampler uses precomputed samples from a sampler factory. */
  class Sampler 
  {
  public:
    ...
    Vec2i getIntegerRaster()
    { return sample.integerRaster; }
    ...
    float getFloat (int dim)
    { return sample.samples1D[dim]; }

    /*! Get the specified additional 2D sample for the current sample. */
    Vec2f getVec2f(int dim)
    { return sample.samples2D[dim]; }

    /*! Get the specified precomputed light sample for the current sample. */
    LightSample getLightSample(int lightSampleId)
    { return sample.lightSamples[lightSampleId]; }
    ...
    const Vec2i& getTileBegin() const
    { return tileBegin; }

    /*! Get coordinates of last pixel in tile. */
    const Vec2i& getTileEnd() const
    { return tileEnd; }
  ...
  protected:
    ...
    PrecomputedSample sample;     //!< The current sample.
  ...
  protected:
    ...
    Random randomNumberGenerator;      //!< Random number generator used by this factory.
